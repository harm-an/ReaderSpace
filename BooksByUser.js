...
export default class BooksByUser extends Component<Props> {

  state={
    view:'Home',
    isLoading: true
  }
  object = {name: this.props.bookName};
  ...
  componentDidMount(){
    GoogleSignin.currentUserAsync().then((user) => {
        ...
        .then((responseJson) => {
          ...
          booksName = responseJson.list;
          //Alert.alert(JSON.stringify(responseJson.list))
  ...
  returnBooks = () => {
    ...
    for(let j = 0; j<booksName.length; j++){
      for(let i = 0; i<booksDB.length; i++){
        if(booksName[j] == JSON.stringify(booksDB[i].asin))
